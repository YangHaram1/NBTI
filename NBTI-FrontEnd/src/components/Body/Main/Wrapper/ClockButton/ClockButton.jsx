...
const ClockButton = ({ type }) => {
  ...
  const fetchAttendanceStatus = useCallback(async () => {
    ...
    try {
      ...
      const { start_date, end_date, clockedIn, clockedOut, isLate } =
        response.data;
      ...
      const isClockInToday =
        startDate && startDate.toISOString().split("T")[0] === today;
      const isClockOutToday =
        endDate && endDate.toISOString().split("T")[0] === today;
  ...
  const handleClockIn = async () => {
    ...
    try {
      ...
      setIsAbsent(false);
      alert(
        isLate ? "지각 기록이 저장되었습니다." : "출근 기록이 저장되었습니다."
      );
        ...
        <div className={styles.timeDisplay}>
          ...
          {type === "clock-out" && (
            <>
              ...
              {isEarlyLeave && (
                <span className={styles.comment}>조기 퇴근</span>
              )}
            </>
          )}
        </div>
